...
// (Fanvue's typing indicator is a no-op for a server-side bot — it only renders
// for a user shown as "online", and there's no API to set presence — so we
// don't bother with it; the read receipt is the one signal that actually shows.)
...
async function reply(
  ...
  // Give the fan the read receipt (blue checkmarks) as we start handling her reply.
  markRead(fanUuid);
